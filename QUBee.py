...
def detection(route):
      ...
      while cmds.next < cmds.count:
        ...
        now = time.time()
        dist = distanceBetween(planePos, route[cmds.next - 4])
                          ...
                          for cn in contours:
                              ...
                              if 4 <= len(approxn) <= 6:
                                  ...
                                  if keepAspectRatio:
                                      ...
                                      t_rotated = cv2.adaptiveThreshold(b_rotated, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                                        cv2.THRESH_BINARY_INV, 11, 2)
                                      e_rotated = cv2.Canny(b_rotated, 50, 100)

                                      # uses the outline to detect the corners for the cropping of the image
                                      (contours, _) = cv2.findContours(e_rotated.copy(), cv2.RETR_EXTERNAL,
...
def recognition(counter):
    ...
    for i in range(self.counter):
        img = cv2.imread("rotated%d.png" % i)
...
def navigation(route, accuracy):
  ...
  #add takeoff command, will be ignored if already in the aircraft
  cmds.add(Command( 0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0, 0, 10))
  
  for i in range(len(route)):
    cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0,0, 0, accuracy, 0, 0, route[i][0], route[i][1], route[i][2]))
  
  #Add dummy waypoint, for monitoring the end of the navigation phase
  cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0,0, 0, accuracy, 0, 0, route[3][0], route[3][1], route[3][2]))
...
def speedLap(wp, accuracy):
  #Loiter while commands are uploading, also need to switch out of auto and back to properly run mission
  vehicle.mode = VehicleMode("LOITER")
  
  cmds = vehicle.commands
  # Upload a command list for the speed lap
  cmds.clear()
  
  #Add commands to go to pre-speed lap point for line-up
  for i in range(2):
    cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0,0, 0, accuracy, 0, 0, wp[i][0], wp[i][1], wp[i][2]))
  #Change throttle to 100% for speed lap
  cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED,0,0, 0, 0, 100,0,0,0,0))
  
  for i in range(len(wp)-2):
    cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0,0, 0, accuracy, 0, 0, wp[i+2][0], wp[i+2][1], wp[i+2][2]))
    #Add a change throttle command after every wp, in case any are missed
    cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED,0,0, 0, 0, 100,0,0,0,0))
  
  #Add dummy waypoint, for monitoring the end of the speed phase
  cmds.add(Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0,0, 0, accuracy, 0, 0, wp[4][0], wp[4][1], wp[4][2]))
...
while cmds.next < cmds.count:
  ...
  #Monitor flight status and print to console every 2s, or every second when close to wp
  now = time.time()
  dist = distanceBetween(planePos, route[cmds.next - 3])
  if dist > vehicle.airspeed *2:
    if now - start > 1:
      print("Distance to next waypoint: %fm" % dist)
      print("Airspeed: %fm/s" % vehicle.airspeed)
